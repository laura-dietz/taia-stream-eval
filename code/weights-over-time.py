...
def createWeightPlot(prefix,intervalRunfiles, entityList, title):
    ...
    for idx,(intervalType, runfiles) in enumerate(intervalRunfiles.items()[:]):
        ...
        for runIdx, evalFile in enumerate(sorted(runfiles[:])):
            ...
            for (intervalLow, intervalUp) in intervalBounds[judgmentLevel][intervalType]:
                    ...
                    correctedValues = [ correctWeighting( 0.5
...
#                        ]
                    ...
                    weightedValues = correctedWeighting
                    #print 'uniform =',np.mean(uniformWeighting), ' corrected=',np.mean(correctedWeighting),' choosing ',np.mean(weightedValues)
                    records.append(createStatsRecord(team, runname, intervalLow, unjudgedAs, judgmentLevel, "weight", np.mean(weightedValues), np.std(weightedValues), intervalType))
    ...
    if args.subplot: plt.savefig("%s%s_teams_over_time_%s_%s.pdf"%(prefix,'weights', judgmentLevelToStr(judgmentLevel),correctedToStr()), bbox_inches='tight')
    plt.clf()
...
def plotWeights():
    ...
    createWeightPlot(evalDir+"all_geo_weights__",intervalRunfiles, entityList, 'all')
    for entity in entityList:
        createWeightPlot(evalDir+entity+"__weights__",intervalRunfiles, [entity], entity)
