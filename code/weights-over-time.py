    ...
    for idx, intervalType in enumerate(['all', 'week', 'day']):
        ...
        for (intervalLow, intervalUp) in intervalBounds[judgmentLevel][intervalType]:
            ...
            if (len(weightedValues) > 0):
                records.append(createStatsRecord(entityListName, intervalLow, unjudgedAs, judgmentLevel, "weight",
        ...
        figfile = "%s%s_%s_teams_over_time_%s_%s.pdf" % (
        prefix, intervalType, 'weights', judgmentLevelToStr(judgmentLevel), correctedToStr())
