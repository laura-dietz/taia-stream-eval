...
teamColors={team:cm.hsv(1. * i/len(allteams),0.5) for i,team in enumerate(np.unique(allteams))}
...
def createPlot(prefix,intervalRunfiles, metric):
...
#                    posvalues = np.array([posTruthsInterval(judgmentLevel, entity, intervalLow, intervalUp) 
#                        for entity in entityList
#                        if isPosIntervalForEntity(judgmentLevel, entity, intervalLow, intervalUp)
#                        ])
        ...
        plt.plot(epochsToDate(np.array(xs)), ys, label=seriesLabel, color=plotcolor, alpha=0.7)
...
#plt.gca().autoscale_view(tight=True, scalex=True, scaley=False)
    plt.xlim(0, 100)
    plt.ylim(0.0, 0.65)
    print 'saving',("%s%s_%s_teams_over_time_%s_%s.pdf"%(prefix,intervalType,metric, judgmentLevelToStr(judgmentLevel), correctedToStr()))
...
createPlot(os.path.expanduser(evalDir)+'overview_4paper_',allIntervalRunfiles, 'Prec@R')
sys.exit()

def plotAll():
    for metric in metrics:
        createPlot(os.path.expanduser(evalDir)+'overview_',allIntervalRunfiles, metric)
